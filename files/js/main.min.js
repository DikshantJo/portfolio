...
function loadWorkData() {
	return new Promise((resolve, error) => {
		fetch("files/js/data.json")
			.then((data) => data.json())
			.then((data) => {
				let workContainer = "#contentContainer .work-list ul";
...
function resetAll() {
	$("#contentContainer .parallax-wrapper.home-back").css("height", "0%");
	$("#contentContainer .title-wrapper *, .nav-wrapper").css("transform", "translateY(200%)");
	$("#contentContainer .parallax-wrapper.home-back img").css("transform", "scale(1.3)");
	$("#contentContainer .h-animate").css("transform", "translateY(100%)");
	$("#contentContainer .parallax-wrapper.home-back, #contentContainer .h-animate, #contentContainer .title-wrapper *, .nav-wrapper").css("opacity", "0");
	$("#contentContainer .load .loader").css("opacity", "0");
	$("#contentContainer .load .loader").css("transform", "translateY(50px)");
	...
	anime({
		targets: "#contentContainer .load .loader",
		opacity:1,
		translateY: "0",
...
function animatePageIn() {
	return new Promise((resolve, error) => {
		...
		Promise.all(promiseArray).then(() => {
			anime({
				targets: "#contentContainer .wrap",
				opacity:0,
				duration: 1000,
				delay: 2000,
				easing: "cubicBezier(0.755, 0.05, 0.855, 0.06)",
				complete: function() {
					// $("#contentContainer .wrap").remove();
					const element = document.querySelector("#contentContainer .wrap");
					// element.remove();
					...
					anime({
						targets: "#contentContainer .parallax-wrapper.home-back",
						height: "100%",
						opacity:1,
						easing: "cubicBezier(0.165, 0.84, 0.44, 1)",
						duration: 1300,
					...
					anime({
						targets: "#contentContainer .parallax-wrapper.home-back img",
						scale: "1",
						opacity:1,
						easing: "cubicBezier(0.165, 0.84, 0.44, 1)",
						duration: 1400,
					...
					setTimeout(function() {
						anime({
							targets: "#contentContainer .h-animate",
							translateY: "0%",
							opacity:1,
							easing: "cubicBezier(0.165, 0.84, 0.44, 1)",
							duration: 1400,
...
function mobileSetup() {
	if (isMobile()) {
		$(".wave-filter, #contentContainer.work-click-area ul").each(function() {
			$(this).addClass("disabled");
		});
		$("#contentContainer .text-content .scroll-msg.subheader").text("Swipe Up");
		$("#work-canvas").remove();
		$("#contentContainer.work-click-area").css("overflow-x", "auto");
		$("#contentContainer.work-click-area").css("overflow-y", "hidden");
		$("#contentContainer.work-click-area .hold-text").remove();
...
function navHandler() {
	var locations = [
		...
		{
			name: 'about',
			location: calcPosition("#contentContainer.work-click-area")
		},
		{	name: 'work',
			location: calcPosition("#contentContainer.about")
		},
		{	name: 'footer',
			location: calcPosition("#contentContainer.footer")
		}
	];
	$("body").data("position", locations[0].name);

	$(window).resize(() => {
		locations[1].location = calcPosition("#contentContainer.work-click-area")
		locations[2].location = calcPosition("#contentContainer.about")
		locations[3].location = calcPosition("#contentContainer.footer")
	});
	...
	function scrollPage(isDown) {
		...
		locations[1].location = calcPosition("#contentContainer.about");
		locations[2].location = calcPosition("#contentContainer.work-click-area");
		locations[3].location = calcPosition("#contentContainer.footer");
...
function workListScroller() {
	...
	let slider = $('#contentContainer .work-list');
	let maxSpeed = 5000;
	...
	$(document).mousemove((e) => {
		...
		let diff = secondPosX - initialMousePosX;
		let calcPosition = initialPos - (maxSpeed * (diff / document.body.clientWidth));
	...
	function calculateEndPos() {
		...
		$("#contentContainer .work-list ul li").each(function() {
...
function hoverTracker() {
	...
	function trackMouse(e) {
		...
		var clickableElement = isHoveringClickable(e);
		if (clickableElement) {
...
function openItem(id,workData) {
	...
	for (let i=0; i<workData[id].roles.length; i++) {
		$("#listing-container .roles").append(`<li>${workData[id].roles[i]}</li>`);
	}
	for (let i=0; i<workData[id].links.length; i++) {
		...
		complete: () => {
			...
			anime({
				...
				complete: () => {
					...
					setTimeout(() => {
						$("#listing-container .work-close-btn").addClass("show");
						$("#listing-container ._SS_dummy").height($("#listing-container .contentContainer").outerHeight(true));
					}, 100);
				}
			});			
		}
...
function workClickListener() {
	...
	$(".work-close-btnI").on("click", () => {
		closeAboutme();
	});


	$(".list-item").click(function(e) {
		openItem(parseInt($(this).data('id'))-1,workData);
	});
	...
	_SS.momentumScroll({
		...
		onScroll: () => {
			...
			if ($("body").data("position") == "about") {
				setTimeout(() => {
					animate("#contentContainer.about .a-animate", 1);
					$("#contentContainer.about .background").css("opacity", "1");
				}, 500);
			}
			if ($("body").data("position") == "footer") {
				setTimeout(() => {
					animate("#contentContainer.footer .a-animate", 1);
					$("#contentContainer.footer .background").css("opacity", "1");
				}, 500);
			}
		}
	});
...
const printInfo = () => {
	aboutMe.data.then((a) => {
	 let myData = {
		 company:a[0].CompanyI,
		 details:a[0].details.description,
		 duration:a[0].duration,
		 roles:a[0].roles,
		 title:a[0].title,
		 links:a[0].links,
		 id:a[0].id,
		 whatILearned:a[0].whatILearned
	 }

		$("#listing-container .scroll-wrapper").scrollTop(0);
		$("#listing-containerI .f-titleI").text(myData.title);
		$("#listing-containerI .f-descriptionI").text(myData.details);
		$("#listing-containerI .f-indexI span").text("0" + myData.id);
		document.querySelector('.f-dateI').textContent = `${myData.company}`
		document.querySelector('.rolesAdded').textContent = `${myData.roles}`
		document.querySelector('.f-s-descriptionI').textContent = `${myData.whatILearned}`;
	
		WORK_ACTIVE = true;
		$(".nav-wrapper .hb-button, .nav-wrapper .logo-icon").addClass("invert");
		$("#listing-containerI .details-wrapperI *, #listing-containerI .primary-image-containerI, #listing-containerI .stats-wrapperI *, #listing-containerI .secondary-image-wrapperI").css("transform", "translateY(10vh)");
		$("#listing-containerI .details-wrapperI *, #listing-containerI .primary-image-containerI, #listing-containerI .stats-wrapperI *, #listing-containerI .secondary-image-wrapperI").css("opacity", "0");
		...
		if (isMobile()) {
			...
			allowAnimate = false;
			workScroll = _SS.momentumScroll({
				root: "#listing-containerI .scroll-wrapperI",
				duration: 1200,
				easing: "cubic-bezier(0.19, 1, 0.22, 1)",
				onScroll: () => {
					_SS.inView("#listing-containerI .stats-wrapperI", {
						inView: () => {
							if (!allowAnimate) allowAnimate = true;
							if (WORK_ACTIVE && allowAnimate) detailsAnimate();
						}
		...
		function detailsAnimate() {
			anime({
				targets: "#listing-containerI .secondary-image-wrapperI",
				opacity: "1",
				translateY: "0vh",
				easing: "cubicBezier(0.25, 1, 0.5, 1)",
				duration: 1200
			...
			anime({
				targets: "#listing-containerI .scroll-sectionI .details-wrapperI *, #listing-containerI .stats-wrapperI *",
				opacity: "1",
				translateY: "0vh",
				easing: "cubicBezier(0.25, 1, 0.5, 1)",
				duration: 1200,
				delay: anime.stagger(100),
				complete: () => {
					$("#listing-containerI .work-close-btnI").css("display", "block");
					setTimeout(() => {
						$("#listing-containerI .work-close-btnI").addClass("show");
					}, 100);
					allowAnimate = false;
				}
			});
		...
		anime({
			targets: "#listing-containerI .content-wrapperI",
			paddingLeft: "7vw",
			easing: "cubicBezier(0.23, 1, 0.32, 1)",
			duration: 1450
		});
		anime({
			targets: "#listing-containerI",
			width: "100vw",
			easing: "cubicBezier(0.86, 0, 0.07, 1)",
			duration: 1200,
			complete: () => {
				...
				anime({
					targets: "#listing-containerI .primary-image-containerI",
					opacity: "1",
					translateY: "0vh",
					easing: "cubicBezier(0.25, 1, 0.5, 1)",
					duration: 1200,
				...
				anime({
					targets: "#listing-containerI .entry-sectionI .details-wrapperI *",
					opacity: "1",
					translateY: "0vh",
					easing: "cubicBezier(0.25, 1, 0.5, 1)",
					duration: 1200,
					...
					complete: () => {
						...
						setTimeout(() => {
							$("#listing-containerI .work-close-btnI").addClass("show");
							$("#listing-containerI ._SS_dummy").height($("#listing-containerI .contentContainerI").outerHeight(true));
							// $("#listing-containerI ._SS_dummy").css('display','none')
						}, 100);
					}
				});			
			}
  ...
  function closeAboutme() {
	if (!allowAnimate) {
		WORK_ACTIVE = false;
		$("#listing-containerI .work-close-btnI").removeClass("show");
		anime({
			targets: "#listing-containerI .primary-image-containerI, #listing-containerI .secondary-image-wrapperI",
			opacity: "0",
			easing: "cubicBezier(0.25, 1, 0.5, 1)",
			duration: 400
		});
		anime({
			targets: "#listing-containerI .details-wrapperI *, #listing-containerI .stats-wrapperI *",
			opacity: "0",
			easing: "cubicBezier(0.25, 1, 0.5, 1)",
			duration: 400,
			complete: () => {
				...
				anime({
					...
					duration: 1200,
					complete: () => {
						$("#listing-containerI .work-close-btnI").css("display", "none");
